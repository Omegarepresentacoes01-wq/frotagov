...
import { CheckCircle, Clock, DollarSign, Wallet, Zap, CalendarCheck, Edit3 } from 'lucide-react';
...
export const FuelStationDashboard: React.FC<Props> = ({ user }) => {
  ...
  const pendingValidations = transactions.filter(t => t.status === TransactionStatus.REQUESTED);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <span className="bg-blue-100 text-blue-800 text-xs font-bold px-2 py-1 rounded-full">{pendingValidations.length} aguardando</span>
          </div>
          ...
          {pendingValidations.length === 0 ? (
            <div className="bg-white p-12 rounded-2xl text-center border-2 border-dashed border-slate-200">
              ...
              <h3 className="text-slate-500 font-medium">Nenhum veículo aguardando no momento.</h3>
            </div>
          ) : (
                  ...
                  {validatingTx === tx.id ? (
                    ...
                    <div className="p-6 bg-blue-50/30">
                      <div className="flex justify-between items-center mb-4 pb-4 border-b border-blue-100">
                        ...
                        <span className="text-sm font-mono bg-white px-2 py-1 rounded border border-blue-200">{tx.vehicleId}</span>
                      </div>
                    ...
                    <div className="p-5 flex flex-col md:flex-row justify-between items-center gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500">Solicitado por: <span className="text-slate-700 font-medium">{tx.driverName}</span></p>
                          <p className="text-xs text-slate-400 mt-0.5">{new Date(tx.requestDate).toLocaleString()}</p>
                        </div>
                      </div>
                      <button 
